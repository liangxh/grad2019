...
对于表中的四个指标，均由CNN达到最好的效果。对于F1值，排第二和第三的分别是CNN配合注意力机制以及GRU配合注意力机制，两者和CNN的F1值差距明显（约0.02）。对于准确率，第二的GRU配合注意力机制和第三的CNN配合注意力机制的数值接近，也和CNN有一定差距（约0.13）。对于正确率，第二为CNN配合注意力机制，数值明显低于CNN（差距约0.01）。对于召回率，由GRU配合注意力机制、BiGRU配合注意力机制、BiLSTM配合注意力机制并排第二，与CNN的准确率差距明显（约0.01）。
