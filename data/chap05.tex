...
对于F1值，CNN达到最好的0.7437 ，其次是的BiGRU配合注意力机制和CNN配合注意力机制，两者和第一的CNN差距较大（约0.04）。对于准确率，同样是CNN的数值最高（0.9245），第二和第三分别是BiGRU配合注意力机制和GRU配合注意力机制，同样地和CNN差距较大（约0.01）。对于正确率，则是GRU配合注意力机制的性能最优（0.7330），其次的GRU和CNN的前者差距较小（仅0.005）。对于召回率，第一位依然是CNN（0.7584），其次是CNN配合注意力机制，和前者差距约0.016 ，第三的BiGRU配合注意力机制则和前两者差距较大（数值低于0.7）。
...
对于面向三轮对话的“开心”、“悲伤”和“愤怒”三分类问题，表~\ref{tab:exp_context_emo_tri_result}和图~\ref{fig:exp_context_emo_tri_result_bar}显示不同卷积神经网络和迭归神经网络在我们模型框架中能达到的性能。注意表中的正确率、召回率、F1值同样对应章节~\ref{ssec:exp_context_emo_eval_metric}中定义的$P_\mu$、$R_\mu$和$F_\mu$。

对于F1值，CNN达到最高数值的0.7571，第二为GRU配合注意力机制，第三为BiGRU配合注意力机制，两者的F1值和CNN的差距都较大（0.01以上）。对于准确率和正确率，均由GRU配合注意力机制达到最好的性能（数值分别为0.9230和0.7343），第二为CNN，准确率和前者接近，但正确率差距较明显（约0.02），第三为BiGRU配合注意力机制。对于召回率，则是GRU的性能最佳（0.8233），第二的LSTM和前者的差距明显（约0.01）。

总的来说，CNN同时在四项指标上达到靠前的性能，GRU配合注意力机制也在三项指标上达到较好的性能，但召回率要远低于CNN（差距约为0.05），故对于面向三轮对话的“开心”、“悲伤”和“愤怒”三分类问题，依然是CNN的整体建模能力最好。
...
对于面向三轮对话的“其他”和“不是其他”二分类问题，表~\ref{tab:exp_context_emo_b_result}和图~\ref{fig:exp_context_emo_b_result_bar}显示不同卷积神经网络和迭归神经网络在我们模型框架中能达到的性能。注意表中的正确率、召回率、F1值同样对应“不是其他”一类的指标。

对于F1值，CNN达到最高数值的0.7571，第二为GRU配合注意力机制，第三为BiGRU配合注意力机制，两者的F1值和CNN的差距都较大（0.01以上）。对于准确率和正确率，均由GRU配合注意力机制达到最好的性能（数值分别为0.9230和0.7343），第二为CNN，准确率和前者接近，但正确率差距较明显（约0.02），第三为BiGRU配合注意力机制。对于召回率，则是GRU的性能最佳（0.8233），第二的LSTM和前者的差距明显（约0.01）。

总的来说，CNN同时在四项指标上达到靠前的性能，GRU配合注意力机制也在三项指标上达到较好的性能，但召回率要远低于CNN（差距约为0.05），故对于面向三轮对话的“其他”和“不是其他”二分类问题，依然是CNN的整体建模能力最好。
