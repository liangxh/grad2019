...
另外，由于我們的系統在经过每一步決策后的中間結果都可认作为一組預测结果，我們观察了每組中間結果在测试集上的性能。表~\ref{tab:exp_irony_det_B_ensemble_result}显示我們系統的中間结果在测试集上的各项指标，注意表中的正确率、召回率、F1值均是针对各类别对应指标数据的宏平均。

表中的第一行“中間结果一”对应直接由一組四分类器進行多数投票的识別结果，其F1值已超过原比赛中排名第一的数值。第二行是基于第二个分类器組的投票对被判定为“没有反讽”和“基于相反语义的反讽”的样本重新进行二选一的识別结果，可见除了召回率以外的三項指标都有所提升，可以推


% 数据集 & 没有反讽 & 基于相反语义的反讽 & 情景反讽 & 其他反讽\\  

表~\ref{tab:exp_irony_det_B_conf_mat_1}
表~\ref{tab:exp_irony_det_B_conf_mat_2}
表~\ref{tab:exp_irony_det_B_conf_mat_3}
表~\ref{tab:exp_irony_det_B_conf_mat_4}
